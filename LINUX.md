...
Consulte enquanto faz o CTF, achando a sua pergunta na tabela abaixo e indo
direto ao ponto. Todos os comandos são para rodar no seu laboratório, então abra
o terminal antes de continuar.
...
O mapa das missões do CTF está [no fim](#o-que-cada-missão-do-ctf-usa).
...
**Caminho absoluto e relativo.** Um caminho que começa com `/` é absoluto e vale
...
| `/proc` | o kernel expondo o estado do sistema, sem arquivos de verdade no disco |
...
Depois de editar, quase sempre é preciso recarregar o serviço, porque o programa
...
`sudo comando` executa aquele comando como root. Você continua na sua conta e pede
...
Serviço é um programa que o sistema mantém rodando, como sshd, nginx e fail2ban.
...
> máquina reiniciar, de modo que o conserto durou até o boot.
...
| Comando | O que faz |
|---|---|
| `sudo apt update` | baixa a lista do que existe, sem instalar nada |
...
que a máquina respondeu dizendo "não tem ninguém nessa porta", ou seja, você chegou
até ela. *Timed out* significa que ninguém respondeu nada, provavelmente por causa de um
...
você fez aqui, como `ls`, `nginx` e `ssh`, roda no espaço de usuário. Um
...
<sub>Achou algo errado ou faltando? Diga no canal da disciplina, porque este material
